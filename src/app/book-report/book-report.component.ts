...
export class BookReportComponent implements OnInit {
  ...
  @ViewChild('datatable') Data: ElementRef;
  ngOnInit() {
    // this.getBookReport()
    this.createForm();
    this.DatePicker()
  ...
  getBookReport(FromDate: string, ToDate: string) {
    debugger
    this.bookReportService.getBookInflowReport(FromDate, ToDate).subscribe(Result => {
      let i = 0
      for (let r of Result) {
        debugger
